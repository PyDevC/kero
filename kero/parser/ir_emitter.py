...
class IREmitter:
    ...
    def emit(self, root: RelNode, module: Module) -> IRQueryMeta:
        ...
        seen_cols: OrderedDict[str, ColumnRef] = OrderedDict()
        ...
        for i, (col_key, col_ref) in enumerate(seen_cols.items(), start=1):
            mlir_elem = self._arrow_to_mlir(col_ref.dtype)
    ...
    def _emit_expr_in_region(
        ...
        if isinstance(node, ColumnRef):
            ...
            mlir_elem = self._arrow_to_mlir(node.dtype)
    ...
    def _collect_columns(
        self, node: RelNode, seen: OrderedDict
    ) -> None:
    ...
    def _collect_columns_expr(self, expr: Expr, seen: OrderedDict) -> None:
